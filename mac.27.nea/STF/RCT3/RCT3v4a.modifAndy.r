...
#          ybar = weighted.mean(log(Calibration$VPA[!is.na(.input[,2])]+1),(1-(((Prediction$Year[year]-Calibration$Year[!is.na(.input[,2])])/tapert)^taper))^taper)
 ...
 #Andrew Campbell 02/04/2019 
    #negative weights for older years than tapert? 
    #according to doc data older than tapert should be ignored
    #so, set negative weights to to zero
    w[w<0]<-0
    #cat(w,"\n")
    #end 02/04/2019
