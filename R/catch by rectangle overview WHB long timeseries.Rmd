...
neafc <-
  ...
  lowcase() %>% 
  rename(rect=icesrect, catch=catchtonnes) %>% 
  dplyr::rename(pnum = month) %>%
  dplyr::mutate(ptype = "M")
...
compare2 <-
  bind_rows(
    catch_by_species_year_country_raw %>% 
      ...
      mutate(zone = ifelse(zone %in% c("GBR"), "EU27", zone)) %>% 
      # mutate(zone = ifelse(zone %in% c("GBR","EU27"), "EU", zone)) %>% 
      mutate(source="datacall"),
    neafc %>% 
      filter(year %in% 2010:2012) %>% 
      mutate(source="neafc")
  ) %>% 
  group_by(source, year, zone) %>% 
...
compare2 %>% 
  ...
  theme_publication() +
  geom_bar(aes(fill=source), stat="identity", position=position_dodge2(width=0.9)) +
  facet_wrap(~zone)
...
Boxplot of relative catch by month for long time series
...
t <-
  ...
  group_by(year) %>%
  mutate(prop = catch/sum(catch,na.rm=TRUE)) %>% 
  mutate(years = cut(year, breaks=seq(1975,2020,5), include.lowest = TRUE, dig.lab=10 ) )
...
t %>% 
  ...
  theme(legend.position="right") +
  geom_line(aes(group=year)) +
  geom_line(data=t2, aes(x=month, y=prop) , colour="blue", size=1.5, inherit.aes = FALSE) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 5L)) +
  scale_x_continuous(breaks = 1:12) +
  labs(y="relative catch (%)") +
  expand_limits(y=0) +
